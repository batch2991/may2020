...
public class SanityTest extends MainClass
{
  ...
  public void sanitytesting(String str)  throws Exception
  {
	  ...
	  FileInputStream fin = new FileInputStream("d:\\sel_dec18\\testcases.xlsx");  //get excel file
      XSSFWorkbook wb = new XSSFWorkbook(fin);   //workbook in the file
      XSSFSheet ws = wb.getSheet("sanitytest");  //sheet in the workbook

      Row row;
      String classname,methodname;
      for(int r=1;r<=ws.getLastRowNum();r++)  //for all the used rows in the sheet
      {
	    row=ws.getRow(r);
	    if(row.getCell(5).getStringCellValue().matches("yes"))
	    {
			...
			Object obj=c.newInstance();  //create object or instance for the class
